...
public class Lite_Basic_premium55_premium79 extends Base {
	...
	private void activate$deac$lte() throws IOException, InterruptedException {
		...
		while (true) {
			try {
				WebElement $AlertCancel = driver
						.findElement(By.xpath("//div[text()='Your Subscription has been cancelled']"));
		...
		while (true) {
			try {
				...
				WebElement $Alertactive = driver.findElement(By.xpath("//div[text()='User account is activated']"));
				if ($Alertactive.isDisplayed()) {
	...
	private void $activateDecfromManageUser() throws InterruptedException {
		...
		while (true) {
			try {
				WebElement $AlertManageruser = driver
						.findElement(By.xpath("//div[text()='Email sent to Email Id with Activation link code.']"));
				// (//div[@id='bootstrap-alert'])[1]/div[1]/div
				if (!$AlertManageruser.getText().isEmpty() && $AlertManageruser.isDisplayed()) {
					System.out.println($AlertManageruser.getText());
					//
					break;
				}
			} catch (Exception e) {
