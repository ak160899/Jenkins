...
public class SelectYourPlanToYearlyToPremiumPlus extends Base {
	...
	@Test(priority = 0)
	private void $selectYourPlan() throws Exception {

		while (true) {
			try {
				WebElement $dismiss = driver
						.findElement(By.xpath("//span[text()='Time To Upgrade!']//following::button[1]"));
				if ($dismiss.isDisplayed()) {
					visbility(driver, $dismiss, 60);
					click($dismiss);
					break;
				}
			} catch (Exception e) {
	...
	private void $activateDecSuspend$premium() throws InterruptedException {
		...
		while (true) {
			try {
				...
				click($baiscinfoFrmMangeuserProced);
				break;
			} catch (Exception e) {
				// TODO: handle exception
			}
