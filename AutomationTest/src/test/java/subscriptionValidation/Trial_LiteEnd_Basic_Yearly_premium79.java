...
public class Trial_LiteEnd_Basic_Yearly_premium79 extends Base {
	...
	private void l() throws InterruptedException, IOException {
		...
		driver = setUp("chrome");
		pom = new com.pageObjeman.PageObjMan(driver);
	...
	@Test(priority = -7, groups = "trial")
	private void trialMnagerUser() throws Exception {
		while (true) {
			try {
		...
		sendkeys(trp16, Sub_Manager.getInstanecSubManager().getInstanceSubReader().$trialUserEmail());
	...
	@Test(priority = -6, groups = "trial")
	private void trialManage() throws Exception {
		while (true) {
			try {
				click(pom.getInstanceSetting().clickSettings);
				break;
		...
		WebElement trp16 = driver.findElement(By.xpath("//input[@id='UserLoginId']"));
		visbility(driver, trp16, 60);
		sendkeys(trp16, Sub_Manager.getInstanecSubManager().getInstanceSubReader().$trialDrEmail());
		// .sendKeys("Akashn1212@gmail.com");
		sleep(2000);
		WebElement trp17 = driver.findElement(By.xpath("//button[@id='user_dropdown']"));
		visbility(driver, trp17, 60);
	...
	@Test(priority = -5, groups = "trial")
	private void $activateDecSuspend$trial() throws InterruptedException {
	...
	@Test(priority = -4, groups = "trial")

	private void activeuserAndPayement$trial$() throws InterruptedException {
	...
	@Test(priority = -3, groups = "trial")
	private void activateDeactivateuser$trial$() throws InterruptedException, IOException {
	...
	private void cardDetails() {
		...
		while (true) {
			try {
				WebElement $editcardui = driver
						.findElement(By.xpath("//button[@onclick='settingUserSubscription.updateCard()']"));
				click($editcardui);
				break;
			} catch (Exception e) {
				// TODO: handle exception
		...
		while (true) {
			try {
				WebElement $lite = driver.findElement(By.xpath("//span[text()='Free']"));
				sleep(2000);
				if ($lite.isDisplayed()) {
					...
					WebElement $next = driver.findElement(By.xpath("(// span[@title='Next'])"));
		...
		try {
			WebElement $upgradeNow = driver.findElement(By.xpath("//button[@id='save_btn_lite']"));
		...
		while (true) {
			try {
				WebElement $returnHomePage = driver
						.findElement(By.xpath("//a[text()='Click here to return to the home Page']"));
	...
	@Test(priority = -1)
	public void liteEnd() throws InterruptedException {
		...
		for (int i = 1; i <= 5; i++) {
			try {
				WebElement $home = driver.findElement(By.xpath("//td[text()='Home']"));
				visbility(driver, $home, 60);
				click($home);
				break;
			} catch (Exception e) {
				// TODO: handle exception
		...
		for (int i = 1; i <= 5; i++) {
			try {
				if (pom.getInstanceSetting().UpgradeLaterPlanLiteKpop.isDisplayed()) {
					click(pom.getInstanceSetting().UpgradeLaterPlanLiteKpop);
					break;
				}
			} catch (Exception e) {
				// TODO: handle exception
	...
	private void $activate$dc$suspendfromManageuser() throws InterruptedException {
		...
		for (WebElement w4 : wl3) {
			...
			if (w4.getText().trim().equals("DEACTIVE") && w4.isDisplayed()) {
				visbility(driver, w4, 60);
				w4.click();
				break;
			}
		}
	...
	private void paymentAndCrdDetails$() throws InterruptedException {
		...
		sleep(2000);
		WebElement $premiumyearlyToggle = driver.findElement(By.xpath("//span[@class='toggle-slider slider-round']"));
		visbility(driver, $premiumyearlyToggle, 60);
		actions("click", $premiumyearlyToggle);
		sleep(4000);
	...
	private void $premiumManageSubscription() throws InterruptedException, IOException {
		...
		while (true) {
			try {
				visbility(driver, pom.getInstanceSetting().$marketplace, 60);
		...
		while (true) {
			try {
				...
				click(pom.getInstanceSetting().$upgradenowineir$);
				break;
			} catch (Exception e) {
				// TODO: handle exception
			}
		...
		while (true) {
			try {
				if (pom.getInstanceSetting().$editUserInpaymentpage.isDisplayed()) {
					click(pom.getInstanceSetting().$editUserInpaymentpage);
					break;
				}
		...
		while (true) {
			...
			try {
				...
				if (pom.getInstanceSetting().$createuserButtonManagersubPage.isDisplayed()) {
					click(pom.getInstanceSetting().$createuserButtonManagersubPage);
					break;
				}
			} catch (Exception e) {
				// TODO: handle exception
