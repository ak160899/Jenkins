...
public class Select_basic_premium55_premium79 extends Base {
	...
	@Test(priority = -1, groups = "select your plan")
	private void $selectYourPlan() {
		...
		while (true) {
			try {
				WebElement $lite = driver.findElement(By.xpath("//span[text()='$ 19']"));
				sleep(2000);
				if ($lite.isDisplayed()) {
	...
	private void activeuserAndPayement$bas$() throws InterruptedException {
		...
		try {
			WebElement $payment = driver.findElement(By.xpath("(//div[@id='invoicePlans'])[2]"));
	...
	private void $premiumManageSubscription() throws InterruptedException, IOException {
		...
		$preplsdc = user_dr.getText();
		System.out.println("PREMIUMPLUS:" + $preplsdc);
	...
	private void lst() throws InterruptedException, AWTException {
		...
		while (true) {
			try {
				...
				WebElement $usersearch = driver.findElement(By
						.xpath("//input[@id='userPartySearch0']//following::ul[1]/li/a/table[1]/tbody/tr/td[1]/span"));
				if (!$usersearch.getText().isEmpty() && $usersearch.isDisplayed()) {
					click($usersearch);

					WebElement $addUser = driver
							.findElement(By.xpath("//input[@id='userPartySearch0']//following::button[2]"));
					click($addUser);
					break;
				}
			} catch (Exception e) {
		...
		while (true) {
			try {
				WebElement $paymentPrePlus = driver
						.findElement(By.xpath("//input[@id='userPartySearch0']//following::button[5]"));
				click($paymentPrePlus);
				break;
			} catch (Exception e) {
				// TODO: handle exception
			}
		...
		while (true) {
			try {
				WebElement $finishpreplus = driver
						.findElement(By.xpath("//button[@onclick='allVasPremiumThankyou.setting();']"));
				if ($finishpreplus.isDisplayed()) {
					click($finishpreplus);
					break;
				}
			} catch (Exception e) {
