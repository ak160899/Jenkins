...
public class UpgradenowToYearly extends Base {
	...
	@Test(priority = 0)
	private void $upgradeNow() throws InterruptedException {

		while (true) {
			try {
				WebElement $dismissupg = driver.findElement(By.xpath("(//button[@title='Dismiss'])[1]"));
				if ($dismissupg.isDisplayed()) {
					visbility(driver, $dismissupg, 60);
					click($dismissupg);
					break;
				}
			} catch (Exception e) {
		...
		while (true) {
			try {
				WebElement $newpat = driver.findElement(By.xpath("(//button[@title='Add new Patient'])[2]"));
				if ($newpat.isDisplayed()) {
					visbility(driver, $newpat, 60);
					click($newpat);
					break;
				}
			} catch (Exception e) {
		...
		while (true) {
			try {
				WebElement $lite = driver.findElement(By.xpath("//span[text()='$ 55']"));
				sleep(2000);
				if ($lite.isDisplayed()) {
		...
		try {
			WebElement $upgradeNow = driver
					.findElement(By.xpath("(//button[@title='Update card and make payment'])[2]"));
	...
	@Test(priority = 1, groups = "premium")
	...
	@Test(priority = 2, groups = "premium")
	...
	@Test(priority = 3, groups = { "premium" })
	...
	@Test(priority = 4, groups = { "premium" })
	...
	@Test(priority = 5, groups = { "premium" })
	private void activateDeactivateuser$premium$() throws InterruptedException, IOException {
		...
		WebElement $acusericon = driver.findElement(By.xpath("//div[@id='menu-active-user']"));
		visbility(driver, $acusericon, 60);
		actions("click", $acusericon);
		sleep(2000);
...
////////$$$$$$$$$$$$$$premium$79....????/////////////
	...
	@Test(priority = 7, groups = { "premiumplus" })
	...
	@Test(priority = 8, groups = { "premiumplus" })
	...
	@Test(priority = 9, groups = { "premiumplus" })
	...
	@Test(priority = 10, groups = { "premiumplus" })
	...
	@Test(priority = 11, groups = { "premiumplus" })
	...
	@Test(priority = 12, groups = { "premiumplus" })
