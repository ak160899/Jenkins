...
public class Calendar_Test extends LaunchBrowser {
	...
	public static void bookAppointment() {
		...
		for (int j = 0; j < pom.getInstanceCalendar().checkUserStatus.size(); j++) {
			if (pom.getInstanceCalendar().checkUserStatus.get(j).isDisplayed()
					&& pom.getInstanceCalendar().checkUserStatus.get(j).getText()
							.equalsIgnoreCase("Doctor/User not working")) {
				System.out.println("USER ENTER TO OPEN WORKING STATUS");

				System.out.println("DOCTOR SCHEDULED IS DISPLYED AT INDEX:" + j);
	...
	public static void passValueToSearchField() {
		...
		for (int i = 0; i < serachField.size(); i++) {
			System.out.println("pass value:" + i);
			if (serachField.get(i).isDisplayed()) {
				System.out.println("DISPLAYED AT:" + i);
	...
	public static void selectStatus() {
		...
		for (int b = 0; b < selectState.size(); b++) {
	...
	public static void doctorUserNotWorking() {
		...
		for (int i = 1; i < setDoctorUserSchedule.size(); i++) {
			if (setDoctorUserSchedule.get(i).isDisplayed()) {
				clickIntercept(setDoctorUserSchedule.get(i), 30);
				break;
			}
		}

		List<WebElement> checkbox = driver.findElements(By.cssSelector("input#is-working-day"));
		for (int i = 0; i < checkbox.size(); i++) {
			if (checkbox.get(i).isDisplayed()) {
				clickIntercept(checkbox.get(i), 30);

				WebElement ampm = driver.findElement(By.xpath("(//div[@id='thru-ampm'])[1]"));
				visbility(driver, ampm, 60);
				clickIntercept(ampm, 30);

				break;
			}
		}

		List<WebElement> save = driver.findElements(By.xpath("//div[@id='save-btn']"));
		System.out.println(save.size());
		for (int i = 0; i < save.size(); i++) {
			if (save.get(i).isDisplayed()) {
				visbility(driver, save.get(i), 20);
				javascriptclick(save.get(i));
				System.out.println("CLICKED SAVE BUTTON SET dr/user");
				break;
			}
		}
	}

	public static void changeDays() {

		for (int i = 0; i < pom.getInstanceCalendar().dayDrops.size(); i++) {
			if (pom.getInstanceCalendar().dayDrops.get(i).isDisplayed()) {
				clickIntercept(pom.getInstanceCalendar().dayDrops.get(i), 30);
				break;
			}
		}

		for (int i = 0; i < pom.getInstanceCalendar().dayDropsCooseDay.size(); i++) {
			if (pom.getInstanceCalendar().dayDropsCooseDay.get(i).isDisplayed()
					&& pom.getInstanceCalendar().dayDropsCooseDay.get(i).getText().equalsIgnoreCase("Today")) {

				clickIntercept(pom.getInstanceCalendar().dayDropsCooseDay.get(i), 30);
				break;
			}
		}
